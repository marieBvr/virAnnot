...
# Reuirements: Ete3 toolkit
# Aims: Create OTUs based on RPS and Blast annotations
"""
import argparse
			...
			for row in rps_reader:
				if headers == 0:
					...
					if row[1] == "no_hit":
						...
						description = row[8]
						superkingdom = row[9]
						match = re.search("Viruses\((\d+\.\d+)\);", superkingdom)
						# if contig is viral then retrieve sequence
						if match:
							viral_portion = float(match.group(1))
							if viral_portion >= options.viral_portion:
...
def _align_sequences(options, hits_collection):
	...
	for cdd_id in hits_collection:
		...
		tree_file = cdd_output + '/seq_aligned.final_tree.nwx'
		file_cluster = cdd_output + '/otu_cluster.csv'
		
		## create alignment for domain with more than 1 contigs
		if count > 1:
			ete_cmd = 'ete3 build -w standard_trimmed_fasttree -a ' + file_seq_to_align + ' -o ' + cdd_output  + '/ete3 --rename-dup-seqnames -v 0'
			log.debug(ete_cmd)
			os.system(ete_cmd)
		
			## retrieve results
			mv_cmd = 'mv ' + cdd_output  + '/ete3/clustalo_default-trimal01-none-fasttree_full/seq_to_align.fasta.final_tree.fa ' +  file_seq_aligned + "\n"
			mv_cmd += 'mv ' + cdd_output  + '/ete3/clustalo_default-trimal01-none-fasttree_full/seq_to_align.fasta.final_tree.nwx ' +  tree_file
			log.debug(mv_cmd)
			os.system(mv_cmd)
		
			## create tree plot with colors
			file_matrix = cdd_output + "/identity_matrix.csv"
			ete_tree_cmd = 'ete_tree.py -f ' + file_seq_aligned + ' -t ' + tree_file + ' -c ' + file_color_config + ' -m ' + file_matrix + ' -o ' + tree_file + '.png'
			log.debug(ete_tree_cmd)
			os.system(ete_tree_cmd)
			...
			otu_cmd = 'seek_otu.R ' + file_matrix + ' ' + file_cluster + ' ' + str(options.perc)
			log.debug(otu_cmd)
			os.system(otu_cmd)
		# only one contig
		else:
...
def _create_html(options, hits_collection):
	...
	for cdd_id in hits_collection:
		...
		file_seq_aligned = cdd_output + '/seq_aligned.final_tree.fa'
		tree_file = cdd_output + '/seq_aligned.final_tree.nwx.png'
		file_cluster = cdd_output + '/otu_cluster.csv'
		file_matrix = cdd_output + "/identity_matrix.csv"
	...
	html_cmd = 'rps2tree_html.py -m ' + map_file_path + ' -o ' + options.output
	log.debug(html_cmd)
	os.system(html_cmd)
...
def _set_options():
	...
	parser.add_argument('-rgb','--rgb-conf', help='Color palette for contigs coloration', action='store', type=str, default='/home/marie/Project/virAnnot/lib/rgb.txt', dest='file_rgb')
