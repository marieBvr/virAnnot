...
# Aims: Convert blast xml output to csv and add taxonomy
#
"""
import argparse
	...
	for blast_record in records:
		for aln in blast_record.alignments:
			...
			for hit in aln.hsps:
				hsp = {}
				if hit_count == 1:
					first_hit_frame = hit.frame[1] # strand
					cumul_hit_identity = hit.identities
					cumul_hit_score = hit.bits # hit score
					cumul_hit_evalue = hit.expect # evalue
					cumul_hit_length = hit.align_length
			...
			try:
				t = taxoniq.Taxon(accession_id=hsp["accession"])
			except KeyError:
				print("Taxid not found for ", hsp["accession"])
			hsp["tax_id"] = t.tax_id
			hsp["taxonomy"] = ';'.join([t.scientific_name for t in reversed(t.ranked_lineage)])
			hsp["organism"] = t.scientific_name
...
def _get_overlap_value(algo, hsp, type, qlength):
	...
	if hit_overlap == None: hit_overlap = 0
...
def _set_options():
	...
	parser.add_argument('-rn','--read-count', help='Tab-delimited file associating seqID with read number.', action='store', required=True, dest='rn_file')
