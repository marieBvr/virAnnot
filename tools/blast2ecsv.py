...
log.info("Init NCBI taxa database")
ncbi = NCBITaxa()
...
def _guess_database(accession):
    ...
    database_mappings_refseq = {'AC': 'nuccore', 'NC': 'nuccore', 'NG': 'nuccore',
        ...
        'XP': 'protein', 'WP': 'protein', 'EF': 'nuccore',
        'OK': 'nuccore', 'AF': 'nuccore', 'LC': 'nuccore',
        'MH': 'nuccore', 'KC': 'nuccore'}
    return database_mappings_refseq[accession[0:2]]
...
def _read_xml(options):
	...
	for blast_record in records:
		for aln in blast_record.alignments:
			...
			if hsp["accession"].isdigit():
				strange_acc = hsp["description"].split("|")
				try:
					db = _guess_database(strange_acc[1])
				except KeyError:
					continue
			else:
